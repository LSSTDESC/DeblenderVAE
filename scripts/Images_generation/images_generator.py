...
def draw_images(galaxies_psf, band, img_size, filter_name,sky_level_pixel, rng_shear=None, real_or_param = 'param'):
    ...
    if rng_shear is None:
        ...
        poissonian_noise = galsim.PoissonNoise(rng_shear, sky_level=sky_level_pixel)        
    ...
    while counter < max_try:
        try:
            ...
            for i, filter_name in enumerate(filter_names_all):
                ...
                r_int = np.random.randint(1000000000000)
                for s, shear in shear_list:
                    rng_shear = galsim.BaseDeviate(r_int)
                    galaxies_psf = [galsim.Convolve([gal.shear(*shear) * coeff_exp[i], PSF[i]]) for gal in galaxies]
