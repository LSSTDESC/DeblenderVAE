...
# Generation function
def image_generator(cosmos_cat_dir, training_or_test, n_years, used_idx=None, nmax_blend=4, max_try=3, mag_cut=1000.):
    """
    ...
    while counter < max_try:
        try:
            ...
            # Shift centered
            #galaxies[0], shift[0] = shift_gal(galaxies[0], method='uniform', max_dx=0.1)
...
def image_generator_2(cosmos_cat_dir, training_or_test, isolated_or_blended, used_idx=None, nmax_blend=4, max_try=3, mag_cut=1000.,method_first_shift='uniform'):
    """
    Return numpy arrays: noiseless and noisy image of single galaxy and of blended galaxies
    ...
    while counter < max_try:
        try:
            ...
            nb_blended_gal = np.random.randint(nmax_blend)+1
            #print('nb_blended_gal = '+str(nb_blended_gal))
            data = {}
            ...
            if isolated_or_blended == 'blended':
                ...
                if not peak_detection_output:
                    ...
                    #print("nb_blended_gal", nb_blended_gal)
                    #print("idx_closest_to_peak", idx_closest_to_peak)
                    #print("idx_closest_to_peak_galaxy", idx_closest_to_peak_galaxy)

                galaxies_psf = [galsim.Convolve([gal.shift(-center_arc_x, -center_arc_y)*coeff_exp[band], PSF[band]]) for gal in galaxies]
    ...
    # For testing, return unormalized images and data
        ...
        data['SNR'] = utils.SNR(galaxy_noiseless, sky_level_pixel, band=6)[1]
        data['SNR_peak'] = utils.SNR_peak(galaxy_noiseless, sky_level_pixel, band=6)[1]
        #print("galaxy_noiseless", galaxy_noiseless)
        #print("blend_noisy", blend_noisy)
        #print("data", data)
        #print("shift", shift)
        return galaxy_noiseless, blend_noisy, data, shift
    else:
