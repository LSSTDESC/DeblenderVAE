...
for icat in trange(N_files):
    ...
    galaxies = []
    df = pd.DataFrame(index=np.arange(N_per_file), columns=['redshift', 'moment_sigma', 'e1', 'e2', 'SNR', 'SNR_peak', 'mag'])

    res = utils.apply_ntimes(cosmos_galaxy_generator, N_per_file, (cosmos_cat_filename, training_or_test, used_idx, 100))
